...
import { MapPin, GraduationCap, Briefcase, Crown, CheckCircle, Heart, Eye } from 'lucide-react'
import { cn, getAge, cmToFeet } from '@/lib/utils'

export default function ProfileCard({
  profile, onInterest, interestSent = false
}: {
  profile: any
  ...
  return (
    <div className={cn('profile-card group', profile.is_premium && 'profile-card-gold')}>
      {/* Photo */}
      <div className="relative overflow-hidden" style={{ height: 220 }}>
        ...
        <div className="absolute bottom-0 inset-x-0 p-4">
          ...
          <p className="text-white/80 text-sm">{age} yrs • {cmToFeet(profile.height_cm || 160)}</p>
        </div>
      </div>
      ...
      <div className="p-4 space-y-2">
        ...
        {profile.gotra && (
          <div className="pt-2 border-t border-stone-100">
            <span className="text-xs text-saffron-700 bg-saffron-50 border border-saffron-100 px-2.5 py-1 rounded-full font-medium">
              गोत्र: {profile.gotra}
            </span>
          </div>
        )}
        ...
        <div className="flex gap-2 pt-1">
          <Link href={`/profiles/${profile.id}`}
            className="flex-1 flex items-center justify-center gap-1.5 bg-saffron-50 hover:bg-saffron-100
                       text-saffron-700 py-2.5 rounded-xl text-sm font-bold transition-colors">
            <Eye className="w-3.5 h-3.5"/> View
          </Link>
          {onInterest && (
            <button onClick={() => onInterest(profile.id)} disabled={interestSent}
              className={cn(
                ...
                interestSent
                  ...
                  : 'bg-gradient-to-r from-maroon-700 to-maroon-600 text-white hover:from-maroon-800 hover:to-maroon-700 active:scale-95'
              )}>
