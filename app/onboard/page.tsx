...
export default function OnboardPage() {
  ...
  useEffect(() => {
    async function init() {
      // Use getSession to avoid auth lock
      ...
      const { data: existing } = await supabase
        .from('profiles')
        .select('id, onboarding_tier, community, gender, name, completeness')
        .eq('user_id', user.id)
        .maybeSingle()

      if (existing) {
        // Profile exists — if completeness >= 40 go to dashboard
        // otherwise let them continue onboarding
        if (existing.completeness >= 40) {
          router.push('/dashboard')
          return
        }
        // Partial profile — pre-fill and let continue
        ...
        setForm(p => ({
          ...
          name:      existing.name?.startsWith('Groom') || existing.name?.startsWith('Bride')
                     ? '' : (existing.name || ''),
        }))
      }
    }
  ...
  function calcScore() {
    let s = 0
    if (form.name && !form.name.startsWith('Groom') && !form.name.startsWith('Bride')) s += 8
  ...
  // Generate a temp name for step 1 so NOT NULL constraint passes
  function getTempName() {
    const comm  = COMMUNITIES.find(c => c.value === form.community)?.label || form.community || 'Member'
    const gend  = form.gender === 'male' ? 'Groom' : 'Bride'
    return `${gend} (${comm})`
  }
  ...
  async function saveToDb(): Promise<boolean> {
    ...
    // Use real name if filled, else temp name to pass NOT NULL
    const nameToSave = form.name?.trim() ? form.name.trim() : getTempName()
    ...
    const payload: any = {
      ...
      marital_status: form.marital_status || 'never_married',
    ...
    try {
      if (profileId) {
        const { error } = await supabase
          .from('profiles').update(payload).eq('id', profileId)
      ...
      console.error('saveToDb exception:', e)
      return false
    }
  }
  ...
  async function handleNext() {
    ...
    if (stepIdx < STEPS.length - 1) {
      ...
      toast.success(score >= 47
        ? 'Profile submitted for review! 🎉'
        : 'Profile saved! Complete more details for better matches.'
      )
  ...
  async function handleSkipAll() {
    // Only save if community is selected (minimum data)
    if (form.community) {
      setSaving(true)
      await saveToDb()
      setSaving(false)
      toast('Progress saved! Complete your profile anytime.', { icon:'💾' })
    }
    router.push('/dashboard?new=1')
  }
        ...
        <div className="max-w-lg mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-stone-400 font-medium">{stepIdx+1}/{STEPS.length}</span>
            </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          ...
          {!currentStep.required && (
            <div className="flex items-center gap-2 mt-2 bg-blue-50 border border-blue-100 px-3 py-2 rounded-xl w-fit">
        ...
        {score > 0 && (
          <div className={`rounded-2xl p-3.5 mb-5 border flex items-center gap-3
            ${score >= 60 ? 'bg-emerald-50 border-emerald-200'
            : score >= 35 ? 'bg-orange-50 border-orange-200'
            : 'bg-red-50 border-red-200'}`}>
            <div className="flex-1">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-semibold text-stone-700">Profile Strength</span>
                <span className={`text-xs font-black
                  ${score>=60 ? 'text-emerald-600' : score>=35 ? 'text-orange-600' : 'text-red-600'}`}>
                  {score}%
                </span>
              </div>
              <div className="h-1.5 bg-white rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all duration-500"
                  style={{ width:`${score}%`,
                    background: score>=60 ? '#22c55e' : score>=35 ? '#f97316' : '#ef4444' }}/>
              </div>
            </div>
            <p className="text-[10px] text-stone-500 max-w-[100px] leading-tight flex-shrink-0">
              {score < 35 ? 'Add more to be visible'
              : score < 60 ? 'Getting better!'
              : '✅ Great profile!'}
            </p>
          </div>
        )}
                    ...
                    <button key={g.v} type="button" onClick={() => set('gender', g.v)}
                      ...
                      <div>
                        <div className={`font-semibold text-sm
                          ${form.gender===g.v ? 'text-orange-700' : 'text-stone-800'}`}>{g.l}</div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    {v:'never_married',l:'Single',   h:'अविवाहित'},
                    {v:'divorced',     l:'Divorced',  h:'तलाकशुदा'},
                    {v:'widowed',      l:'Widowed',   h:'विधवा'},
                  ].map(m => (
                    <button key={m.v} type="button" onClick={() => set('marital_status', m.v)}
                      className={`py-2.5 rounded-xl border text-xs font-medium transition-all
                        ${form.marital_status===m.v
                          ? 'bg-orange-50 border-orange-400 text-orange-700'
                          : 'border-stone-200 text-stone-600'}`}>
                      {m.l}
                      <div className="text-[9px] opacity-60">{m.h}</div>
                    </button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <select value={form.gotra} onChange={e => set('gotra', e.target.value)} className="input">
                  ...
                  {GOTRA?.map((g: any) => (
                    <option key={g?.value||g} value={g?.value||g}>{g?.label||g}</option>
                  ))}
                </select>
              </div>
            </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-orange-800">
                      Profiles with photo get 5x more matches
                    </p>
                    <p className="text-xs text-orange-700/80 mt-1">
                      Photos visible only to verified members. Up to 5 photos.
                    </p>
                  </div>
                </div>
              </div>
                  ...
                  <div key={i} className="aspect-square rounded-2xl overflow-hidden relative border-2 border-orange-200">
                    ...
                    <button onClick={() => {
                      ...
                      if (form.photo_url === url)
                        set('photo_url', photos.filter(ph => ph !== url)[0] || '')
                    }}
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={form.education} onChange={e => set('education', e.target.value)} className="input">
                  ...
                  {EDUCATION?.map((e: any) => (
                    <option key={e?.value||e} value={e?.value||e}>{e?.label||e}</option>
                  ))}
                </select>
              </div>
              <div>
              ...
              <div>
                ...
                <select value={form.height_cm} onChange={e => set('height_cm', e.target.value)} className="input">
                  ...
                  {HEIGHTS?.map((h: any) => (
                    <option key={h?.value||h} value={h?.value||h}>{h?.label||h}</option>
                  ))}
                </select>
              </div>
              ...
              <div className={`rounded-2xl p-4 border-2 mt-2
                ${score>=60 ? 'bg-emerald-50 border-emerald-300'
                : score>=40 ? 'bg-orange-50 border-orange-300'
                : 'bg-red-50 border-red-200'}`}>
                <div className="flex items-center justify-between mb-2">
                  <span className="font-bold text-stone-800 text-sm">Your Profile Score</span>
                  <span className={`font-black text-xl
                    ${score>=60 ? 'text-emerald-600' : score>=40 ? 'text-orange-600' : 'text-red-600'}`}>
                    {score}%
                  </span>
                </div>
                ...
                <p className={`text-xs font-medium
                  ${score>=60 ? 'text-emerald-700' : score>=40 ? 'text-orange-700' : 'text-red-600'}`}>
                  {score >= 60
          ...
          {STEPS.map((s, i) => (
            <button key={s.id} onClick={() => i < stepIdx && setStepIdx(i)}
              className={`transition-all duration-300
                ${i === stepIdx   ? 'w-6 h-2 bg-orange-500 rounded-full'
                : i < stepIdx     ? 'w-2 h-2 bg-emerald-400 rounded-full'
                : 'w-2 h-2 bg-stone-200 rounded-full'}`}/>
          ))}
        </div>

        <p className="text-center text-[11px] text-stone-400 mt-3">
          💾 Auto-saved · Complete your profile anytime from dashboard
        </p>
      </div>
    </div>
  )
